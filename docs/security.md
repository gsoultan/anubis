...
| **Operator passwords could never move off an old KDF cost** | **Fixed** (2026-09-20) | The same line discarded `needsRehash`. Nothing in the API changes a platform password — there is no self-service change and no admin reset — so a hash written at install time kept that day's iteration count for the life of the installation. A successful login is the only migration path there is, and it now takes it, guarded on the old hash so it cannot resurrect a password that changed underneath it. `scripts/check/kdf-rehash.sh` fails the build if any `kdf.Verify` discards the flag again. **Still open, and a product decision rather than a defect: there is no way to rotate an operator password at all.** The remedy for a suspected compromise is to disable the account and create another, which loses its assignments. |
