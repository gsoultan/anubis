...
| Uniform login timing | `TestLoginTimingDoesNotRevealUserExistence`, `TestPlatformLoginTimingDoesNotRevealOperatorExistence` | Existing vs unknown user: median 49.2 ms vs 48.8 ms — 0.6% apart, both paying full KDF cost. **Finding, 2026-09-20: that was one plane of two.** The test drove the tenant login usecase; the control plane has its own, and `PlatformUserByUsername` returns an EMPTY hash for an unknown operator while `kdf.Verify` parsed before doing any work — so an operator who exists took **51.9 ms and one who does not took 0.35 ms**, a 147× gap that makes every operator username in an installation discoverable. Rate limits do not blunt it: enumeration varies the username, so the per-account limit never trips. Fixed at the choke point rather than the call site — an unreadable hash now costs a full dummy derivation inside `kdf.Verify`, so the property holds for any surface that verifies a password, including ones not written yet. Measured after: 51.4 ms vs 50.8 ms, 1.1% apart |
...
| A realm that requires a factor can enforce it | `TestEnrolOrDenyRollout`, `TestAGrantCannotReplaceAnEnrolledFactor`, `internal/identity/domain` stance suite | The switch is a date, not a flag, because a flag is a lockout: `realms.factor_enrolment_deadline`. The e2e test walks all four states on a live server — no deadline (unchanged behaviour, which is what stops an upgrade locking out every existing realm), inside the grace period (signed in *and* warned), past it (refused, with a grant token), and after enrolling with that grant (challenged for the factor they now hold). The grant is refused if a factor is already enrolled, so a leaked one cannot replace an authenticator  **Finding, 2026-09-20: it was enforced at one door.** Every assertion above drives `AuthService.Login`; `EnrolmentStanceFor` had exactly one caller in the tree, in the interactor, and the hosted sign-in page asked only whether a factor was *enrolled*. So a realm past its deadline refused every API client and admitted the same member through a browser — the door most humans use, and the population the rollout was written for. `TestBrowserLoginHonoursTheEnrolmentDeadline` drives the page instead, and checks both directions: refused past the deadline, still signed in inside the grace period, because closing the hole by refusing every un-enrolled member would satisfy the first assertion and lock out every realm that ever listed a factor. The fix was to stop having two implementations: see the structural row in [security.md](security.md#known-limitations). |
...
| ~~**No hosted enrolment page**~~ | **Built** (2026-09-20). The hosted page spends an enrol grant on the visitor's behalf at both ends of the policy. Refused past the deadline, it renders the setup key and the `otpauth://` link, takes the code, shows the recovery codes once, and the next sign-in is challenged for the factor rather than refused for lacking it. Inside the grace period it signs the member in and then says so — naming the date and the factor, offering to set it up there, and always skippable, because a grace period that blocks is not a grace period. The grant never reaches the browser: the form carries an opaque single-use handle (`one_time_tokens.kind = 'browser_enrol'`, migration 0050) and "continue" re-enters `/v1/authorize`, so PKCE and redirect_uri validation stay where they already live. ~~Remaining: no QR code.~~ **Built** (2026-09-21): `internal/platform/qr` — byte mode, level M, versions 1–10, stdlib only. Inline SVG, so the page still fetches nothing and its `default-src 'none'` CSP is untouched. The Reed-Solomon half is checked against the specification's own worked example and its published generator polynomials; the format and version strings against their published values and their BCH minimum distances; placement end-to-end by a decoder in the test file. **Not verified by a camera** — the page shows the key beside the symbol, so a symbol that will not scan costs a person the typing they would have done anyway. See [enrolment-rollout.md](enrolment-rollout.md#both-doors) |

Nothing above is an unproven claim. Two are decisions with their reasoning
written down, one is a measured ceiling with the cheaper of its two answers
named, and the last was a missing QR encoder, now written.
