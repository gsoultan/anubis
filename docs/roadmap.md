...
| **No hosted enrolment page** | **Open** (found 2026-09-20). `auth_pages.kind` allows `signin` and `signout` only. The API door answers an overdue member with a grant token to enrol against; the browser has nowhere to spend one, so it refuses and names the missing factor. A browser-only member also gets no grace-period warning, because that response redirects straight back to the application. Neither is a security gap — the refusal is correct at both doors — but it means an enrol-or-deny rollout has to be driven by operators for anyone who only uses SSO. See [enrolment-rollout.md](enrolment-rollout.md#both-doors-and-what-the-browser-cannot-do-yet) |
...
named, and the last is a product gap with no security consequence, found by
closing one that had.
