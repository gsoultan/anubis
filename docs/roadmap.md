...
| The sign-in builder configures the page that is actually served | `internal/auth/adapter/http/page_resolution_test.go`, and the hosted page rendered on a running server | **Finding: it did not.** The console wrote a FLAT config — `logo_text`, `brand_color`, `theme` — through the legacy RPCs into `signin_pages`, while the template rendered a NESTED one — `brand.logo_url`, `copy.subheading`, `features.*` — out of `auth_pages`. Migration 0024 moved pages between the two and the console was never moved with it, so roughly half of what the builder saved was written and never read back. That is why the company logo could not be found: `brand.logo_url` is rendered by the template and validated by pagecfg, and the builder had no input for it at all — "Logo text" is `brand.title`, the fallback initial. The preview mirrored the flat shape too, so it drew something the hosted page would never produce. Both now read pagecfg's own schema, and the legacy RPCs are deprecated with `anubis_deprecated_rpc_total` counting whether anything still calls them |
| Internal, partner and public can have different doors | `TestApplicationBeatsRealm`, `TestRealmBeatsTheDefault`, `TestNoRealmCodeSkipsTheRealmLookup`, and the two realms rendered on a running server | Realm kinds were invisible to the page system: `auth_pages` could bind to an application or be the tenant default, and that was all. The realm was already known at render time from `?realm=`, so this is a binding and one step in resolution — inserted between application and default so **nothing that already resolved changes**. The order *is* the feature, so every step is pinned; the test asserting an application still beats a realm was checked by swapping the two blocks, which is the plausible mistake. A page binds to an application OR a realm, never both, because whichever precedence was chosen would surprise somebody and the database refusing the row is cheaper than a rule nobody remembers |
