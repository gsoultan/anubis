...
| ~~**No hosted enrolment page**~~ | **Built** (2026-09-20). The hosted page now spends an enrol grant on the visitor's behalf: refused past the deadline, it renders the setup key and the `otpauth://` link, takes the code, shows the recovery codes once, and the next sign-in is challenged for the factor rather than refused for lacking it. The grant never reaches the browser — the form carries an opaque single-use handle (`one_time_tokens.kind = 'browser_enrol'`, migration 0050). **Still open, smaller: the grace-period warning has nowhere to go.** A member inside the grace period signs in and is redirected straight back to the application, so the browser tells them nothing until the day it refuses them; the API returns the deadline and the missing factors on every sign-in. And there is no QR code — drawing one without a third-party library is a feature of its own under [ADR-0002](adr/0002-dependency-policy.md), so the key is typed or tapped. See [enrolment-rollout.md](enrolment-rollout.md#both-doors) |
...
named, and the last is the remainder of a product gap that closing a security
hole exposed: a member in a grace period is not warned by the browser, only by
the API.
