#!/usr/bin/env bash
# Record one soak reading for the raorm migration (raorm docs/PRODUCTION-READINESS.md
...
# docs/soak-raorm.md, and says plainly when one could not be read rather than
# recording a zero.
set -euo pipefail
cd "$(dirname "$0")/.."

: "${ANUBIS_DB_URL:?set ANUBIS_DB_URL — the soak reads the live dev database}"
OUT="docs/soak-raorm.md"
STAMP="$(date -u '+%Y-%m-%d %H:%M UTC')"

# 1+2. Latency through pgx and through the raorm repository, plus the shape
# counts, all from the integration suite that already measures them.
LOG="$(mktemp)"
trap 'rm -f "$LOG"' EXIT
go test -count=1 -tags integration ./test/integration/ -v \
  -run 'TestAuthorizeLatencyBudget|TestRaormSlice_AuthorizeLatencyBudget|TestRaormFull_VaryingValuesDoNotMintShapes' \
...
raorm_p95="$(grep -o 'authorize over raorm repository: .*p95=[^ ]*' "$LOG" | grep -o 'p95=[^ ]*' | cut -d= -f2 || true)"
...
printf '| %s | %s | %s | %s | %s | %s | %s |\n' \
  "$STAMP" "${pgx_p95:-?}" "${raorm_p95:-?}" "${shapes:-?}" "${flushes:-?}" "$rss" "$drift" >> "$OUT"
