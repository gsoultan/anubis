#!/usr/bin/env bash
# Regenerate all generated code: proto -> Go (gen/go), proto -> TS
# (ui/src/gen). Generated output is committed; CI re-runs this and fails on
# drift.
#
# storm's generators are NOT here: they need a live database to PREPARE
# against, so they run in scripts/ci/backend-suite.sh where one exists.
