...
func (q Query) IsSystemEq(v bool) Query                { return q.Where(IsSystem.Eq(v)) }
func (q Query) IsSystemNotEq(v bool) Query             { return q.Where(IsSystem.NotEq(v)) }
...
func (q Query) DescriptionEqLower(v string) Query      { return q.Where(Description.EqLower(v)) }
...
func (q Query) AssignableAtContains(v ...string) Query { return q.Where(AssignableAt.Contains(v...)) }
func (q Query) AssignableAtContainedBy(v ...string) Query {
	return q.Where(AssignableAt.ContainedBy(v...))
}
func (q Query) AssignableAtOverlaps(v ...string) Query { return q.Where(AssignableAt.Overlaps(v...)) }
func (q Query) AllowedRealmKindsContains(v ...string) Query {
	return q.Where(AllowedRealmKinds.Contains(v...))
}
func (q Query) AllowedRealmKindsContainedBy(v ...string) Query {
	return q.Where(AllowedRealmKinds.ContainedBy(v...))
}
...
var orderTable = [nCols][4]string{
	...
	{ // is_system
		"\"is_system\"",
		"\"is_system\" DESC",
		"\"is_system\" ASC NULLS FIRST",
		"\"is_system\" DESC NULLS LAST",
	},
	{ // name
		"\"name\"",
		"\"name\" DESC",
	...
	{ // assignable_at
		"\"assignable_at\"",
		"\"assignable_at\" DESC",
		"\"assignable_at\" ASC NULLS FIRST",
		"\"assignable_at\" DESC NULLS LAST",
	},
	{ // allowed_realm_kinds
		"\"allowed_realm_kinds\"",
		"\"allowed_realm_kinds\" DESC",
...
func scan(rv [][]byte, r *Row, sl *runtime.Slab) error {
	...
	r.IsSystem = runtime.Bool(rv[5])
	r.Name = sl.Str(rv[6])
	r.Description = sl.Str(rv[7])
	r.AssignableAt, decErr = runtime.TextArray(rv[8], sl)
	if decErr != nil {
		return decErr
	}
	r.AllowedRealmKinds, decErr = runtime.TextArray(rv[9], sl)
	if decErr != nil {
		return decErr
	}
...
var exprFrags = [nUpdatable]runtime.Frag{
	...
	{},                                      // is_system has no server-side form
	{},                                      // name has no server-side form
	{},                                      // description has no server-side form
	{},                                      // assignable_at has no server-side form
	{},                                      // allowed_realm_kinds has no server-side form
	{A: "\"deprecated_at\" = now()", B: ""}, // deprecated_at = the database's clock
}
...
const updReturning = " RETURNING \"id\", \"created_at\", \"updated_at\", \"tenant_id\", \"application_id\", \"is_system\", \"name\", \"description\", \"assignable_at\", \"allowed_realm_kinds\", \"deprecated_at\""
...
func (m *Mut) SetAssignableAt(v []string) {
	m.row.AssignableAt = v
	m.dirty |= dAssignableAt
	m.expr &^= dAssignableAt
}

func (m *Mut) SetAllowedRealmKinds(v []string) {
	m.row.AllowedRealmKinds = v
	m.dirty |= dAllowedRealmKinds
	m.expr &^= dAllowedRealmKinds
}

func (m *Mut) SetDeprecatedAt(v time.Time) {
	m.row.DeprecatedAt = runtime.Null[time.Time]{V: v, Valid: true}
	m.dirty |= dDeprecatedAt
	m.expr &^= dDeprecatedAt
}

// SetDeprecatedAtNow assigns the database's clock, not this process's.
//
// Binding time.Now() instead would record when THIS server
// thought it was. Servers skew, so rows written seconds apart
// can land out of order, and an event stamped by a fast clock
// reads as preceding the thing that caused it.
func (m *Mut) SetDeprecatedAtNow() {
	m.expr |= dDeprecatedAt
	m.dirty &^= dDeprecatedAt
}
...
func (n *Ins) SetAssignableAt(v []string) {
	n.row.AssignableAt = v
	n.set |= iAssignableAt
}

func (n *Ins) SetAllowedRealmKinds(v []string) {
	n.row.AllowedRealmKinds = v
...
// OnConflictTenantIDName upserts on the unique index over (tenant_id, name).
//
// The row that already exists keeps every column this insert did
// not assign. Follow with DoNothing() to leave it untouched
// entirely.
func (n *Ins) OnConflictTenantIDName() *Ins {
	n.conflict = 4
	return n
}
...
var assignFor = map[string]string{
	...
	"is_system":           "\"is_system\" = EXCLUDED.\"is_system\"",
...
func (n *Ins) Insert(ctx context.Context, ex runtime.Executor) (Row, error) {
	...
	for i := 0; i < nInsertable; i++ {
		...
		case 5:
			args = append(args, n.row.IsSystem)
		case 6:
...
func InsertAll(ctx context.Context, ex runtime.Executor, rows []Row) (int64, error) {
	...
	return ex.CopyFrom(ctx, "roles", copyCols, src)
}
...
func (m *Mut) UpdateOp() (runtime.BatchOp, bool) {
	...
	for i := 0; i < nUpdatable; i++ {
		...
		case 3:
			args = append(args, m.row.IsSystem)
		case 4:
