...
func source(kind, cfg string) repository.SyncSourceRecord {
	return repository.SyncSourceRecord{Kind: kind, Axis: "cost_center", Config: []byte(cfg)}
}
...
func TestHTTPFetch(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_ = json.NewEncoder(w).Encode([]repository.SyncFeedRow{
...
func TestDBTableRejectsInjection(t *testing.T) {
	...
	for _, cfg := range cases {
		...
		if code := domain.AsError(err).Code; code != domain.ErrInvalidArgument.Code {
