...
type Row struct {
	GrantedAt     time.Time
	WithdrawnAt   runtime.Null[time.Time]
...
var (
	GrantedAt     = TimeCol{0}
	WithdrawnAt   = NullTimeCol{1}
	ExpiresAt     = NullTimeCol{2}
	ID            = UUIDCol{3}
	TenantID      = UUIDCol{4}
	IdentityID    = UUIDCol{5}
	Purpose       = TextCol{6}
	PolicyVersion = TextCol{7}
	Evidence      = JSONCol{8}
)
...
func (q Query) WithdrawnAtEq(v time.Time) Query          { return q.Where(WithdrawnAt.Eq(v)) }
func (q Query) WithdrawnAtNotEq(v time.Time) Query       { return q.Where(WithdrawnAt.NotEq(v)) }
func (q Query) WithdrawnAtGt(v time.Time) Query          { return q.Where(WithdrawnAt.Gt(v)) }
func (q Query) WithdrawnAtGte(v time.Time) Query         { return q.Where(WithdrawnAt.Gte(v)) }
func (q Query) WithdrawnAtLt(v time.Time) Query          { return q.Where(WithdrawnAt.Lt(v)) }
func (q Query) WithdrawnAtLte(v time.Time) Query         { return q.Where(WithdrawnAt.Lte(v)) }
func (q Query) WithdrawnAtIsNull() Query                 { return q.Where(WithdrawnAt.IsNull()) }
func (q Query) WithdrawnAtIsNotNull() Query              { return q.Where(WithdrawnAt.IsNotNull()) }
...
func (q Query) PolicyVersionEq(v string) Query           { return q.Where(PolicyVersion.Eq(v)) }
func (q Query) PolicyVersionNotEq(v string) Query        { return q.Where(PolicyVersion.NotEq(v)) }
func (q Query) PolicyVersionGt(v string) Query           { return q.Where(PolicyVersion.Gt(v)) }
func (q Query) PolicyVersionGte(v string) Query          { return q.Where(PolicyVersion.Gte(v)) }
func (q Query) PolicyVersionLt(v string) Query           { return q.Where(PolicyVersion.Lt(v)) }
func (q Query) PolicyVersionLte(v string) Query          { return q.Where(PolicyVersion.Lte(v)) }
func (q Query) PolicyVersionEqLower(v string) Query      { return q.Where(PolicyVersion.EqLower(v)) }
func (q Query) PolicyVersionLike(v string) Query         { return q.Where(PolicyVersion.Like(v)) }
func (q Query) PolicyVersionILike(v string) Query        { return q.Where(PolicyVersion.ILike(v)) }
func (q Query) PolicyVersionIn(v ...string) Query        { return q.Where(PolicyVersion.In(v...)) }
func (q Query) PolicyVersionNotIn(v ...string) Query     { return q.Where(PolicyVersion.NotIn(v...)) }
func (q Query) EvidenceContains(v runtime.JSON) Query    { return q.Where(Evidence.Contains(v)) }
func (q Query) EvidenceContainedBy(v runtime.JSON) Query { return q.Where(Evidence.ContainedBy(v)) }
func (q Query) EvidenceHasAnyKey(v ...string) Query      { return q.Where(Evidence.HasAnyKey(v...)) }
func (q Query) EvidenceHasAllKeys(v ...string) Query     { return q.Where(Evidence.HasAllKeys(v...)) }

const selectPrefix = `SELECT "granted_at", "withdrawn_at", "expires_at", "id", "tenant_id", "identity_id", "purpose", "policy_version", "evidence" FROM "consents"`
const countPrefix = `SELECT count(*) FROM "consents"`
const existsPrefix = `SELECT 1 FROM "consents"`
...
var orderTable = [nCols][4]string{
	...
	{ // withdrawn_at
		"\"withdrawn_at\"",
		"\"withdrawn_at\" DESC",
		"\"withdrawn_at\" ASC NULLS FIRST",
		"\"withdrawn_at\" DESC NULLS LAST",
	},
	{ // expires_at
		"\"expires_at\"",
		"\"expires_at\" DESC",
	...
	{ // policy_version
		"\"policy_version\"",
		"\"policy_version\" DESC",
		"\"policy_version\" ASC NULLS FIRST",
		"\"policy_version\" DESC NULLS LAST",
	},
	{ // evidence
		"\"evidence\"",
		"\"evidence\" DESC",
		"\"evidence\" ASC NULLS FIRST",
		"\"evidence\" DESC NULLS LAST",
	},
}
...
var fragTable = [9][28]runtime.Frag{
	...
	{ // withdrawn_at
		...
		{},
		{},
		{A: "\"withdrawn_at\" IS NULL", B: ""},
		{A: "\"withdrawn_at\" IS NOT NULL", B: ""},
		{},
		{},
	},
	{ // expires_at
	...
	{ // policy_version
		...
		{A: "\"policy_version\" < $", B: ""},
		{A: "\"policy_version\" <= $", B: ""},
		{A: "lower(\"policy_version\") = lower($", B: ")"},
		{A: "\"policy_version\" LIKE $", B: ""},
		{A: "\"policy_version\" ILIKE $", B: ""},
		{},
		{},
		{},
		{},
		{},
		{A: "\"policy_version\" = ANY($", B: ")"},
		{A: "\"policy_version\" <> ALL($", B: ")"},
		{},
		{},
		{},
		{},
...
var defaultOrder = [1]runtime.Tok{
	runtime.MakeOrder(runtime.Asc, 3), // id
}
...
func scan(rv [][]byte, r *Row, sl *runtime.Slab) error {
	r.GrantedAt = runtime.Timestamptz(rv[0])
	r.WithdrawnAt = runtime.Nullable(rv[1], runtime.Timestamptz)
	r.ExpiresAt = runtime.Nullable(rv[2], runtime.Timestamptz)
	copy(r.ID[:], rv[3])
	copy(r.TenantID[:], rv[4])
	copy(r.IdentityID[:], rv[5])
	r.Purpose = sl.Str(rv[6])
	r.PolicyVersion = sl.Str(rv[7])
	r.Evidence = runtime.JSON(runtime.JSONB(rv[8], sl))
	return nil
}
...
const insertSQL = `INSERT INTO "consents" ("granted_at", "withdrawn_at", "expires_at", "id", "tenant_id", "identity_id", "purpose", "policy_version", "evidence") VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9) RETURNING "granted_at", "withdrawn_at", "expires_at", "id", "tenant_id", "identity_id", "purpose", "policy_version", "evidence"`

const updatePrefix = `UPDATE "consents" SET `
const deletePrefix = `DELETE FROM "consents"`

// Dirty bits. One per updatable column; the set of them is an UPDATE's
// identity, exactly as a token stream is a SELECT's.
const (
	dGrantedAt     uint64 = 1 << 0
	dWithdrawnAt   uint64 = 1 << 1
	dExpiresAt     uint64 = 1 << 2
	dTenantID      uint64 = 1 << 3
	dIdentityID    uint64 = 1 << 4
	dPurpose       uint64 = 1 << 5
	dPolicyVersion uint64 = 1 << 6
	dEvidence      uint64 = 1 << 7
)
...
var exprFrags = [nUpdatable]runtime.Frag{
	{A: "\"granted_at\" = now()", B: ""},   // granted_at = the database's clock
	{A: "\"withdrawn_at\" = now()", B: ""}, // withdrawn_at = the database's clock
	...
	{},                                     // policy_version has no server-side form
	{},                                     // evidence has no server-side form
}
...
const (
	iGrantedAt     uint64 = 1 << 0
	iWithdrawnAt   uint64 = 1 << 1
	iExpiresAt     uint64 = 1 << 2
	iID            uint64 = 1 << 3
	iTenantID      uint64 = 1 << 4
	iIdentityID    uint64 = 1 << 5
	iPurpose       uint64 = 1 << 6
	iPolicyVersion uint64 = 1 << 7
	iEvidence      uint64 = 1 << 8
)
...
const insPrefix = "INSERT INTO \"consents\""
const insReturning = " RETURNING \"granted_at\", \"withdrawn_at\", \"expires_at\", \"id\", \"tenant_id\", \"identity_id\", \"purpose\", \"policy_version\", \"evidence\""
...
const updReturning = " RETURNING \"granted_at\", \"withdrawn_at\", \"expires_at\", \"id\", \"tenant_id\", \"identity_id\", \"purpose\", \"policy_version\", \"evidence\""
...
func (m *Mut) SetGrantedAt(v time.Time) {
	m.row.GrantedAt = v
	m.dirty |= dGrantedAt
	m.expr &^= dGrantedAt
}

// SetGrantedAtNow assigns the database's clock, not this process's.
//
...
func (m *Mut) SetGrantedAtNow() {
	m.expr |= dGrantedAt
	m.dirty &^= dGrantedAt
}

func (m *Mut) SetWithdrawnAt(v time.Time) {
	m.row.WithdrawnAt = runtime.Null[time.Time]{V: v, Valid: true}
	m.dirty |= dWithdrawnAt
	m.expr &^= dWithdrawnAt
}

// SetWithdrawnAtNow assigns the database's clock, not this process's.
//
// Binding time.Now() instead would record when THIS server
// thought it was. Servers skew, so rows written seconds apart
// can land out of order, and an event stamped by a fast clock
// reads as preceding the thing that caused it.
func (m *Mut) SetWithdrawnAtNow() {
	m.expr |= dWithdrawnAt
	m.dirty &^= dWithdrawnAt
}

// SetWithdrawnAtNull writes SQL NULL. It is a separate method because a
// zero value and an absent value are different facts.
func (m *Mut) SetWithdrawnAtNull() {
	m.row.WithdrawnAt = runtime.Null[time.Time]{}
	m.dirty |= dWithdrawnAt
	m.expr &^= dWithdrawnAt
}
...
func (m *Mut) SetPolicyVersion(v string) {
	m.row.PolicyVersion = v
	m.dirty |= dPolicyVersion
	m.expr &^= dPolicyVersion
}

func (m *Mut) SetEvidence(v runtime.JSON) {
	m.row.Evidence = v
	m.dirty |= dEvidence
	m.expr &^= dEvidence
}
...
func (n *Ins) SetGrantedAt(v time.Time) {
	n.row.GrantedAt = v
	n.set |= iGrantedAt
}

func (n *Ins) SetWithdrawnAt(v time.Time) {
	n.row.WithdrawnAt = runtime.Null[time.Time]{V: v, Valid: true}
	n.set |= iWithdrawnAt
}

// SetWithdrawnAtNull writes SQL NULL explicitly, which is not the same as
// leaving the column unset and taking its default.
func (n *Ins) SetWithdrawnAtNull() {
	n.row.WithdrawnAt = runtime.Null[time.Time]{}
	n.set |= iWithdrawnAt
}
...
func (n *Ins) SetPolicyVersion(v string) {
	n.row.PolicyVersion = v
	n.set |= iPolicyVersion
}

func (n *Ins) SetEvidence(v runtime.JSON) {
	n.row.Evidence = v
	n.set |= iEvidence
}
...
func assignable(i uint8, mask uint64) []string {
	...
	case 0:
		if mask&(1<<0) != 0 {
			set = append(set, "granted_at")
		}
		if mask&(1<<1) != 0 {
			set = append(set, "withdrawn_at")
		}
		if mask&(1<<2) != 0 {
		...
		if mask&(1<<7) != 0 {
			set = append(set, "policy_version")
		}
		if mask&(1<<8) != 0 {
			set = append(set, "evidence")
		}
	}
	return set
}
...
var assignFor = map[string]string{
	"granted_at":     "\"granted_at\" = EXCLUDED.\"granted_at\"",
	"withdrawn_at":   "\"withdrawn_at\" = EXCLUDED.\"withdrawn_at\"",
	...
	"policy_version": "\"policy_version\" = EXCLUDED.\"policy_version\"",
	"evidence":       "\"evidence\" = EXCLUDED.\"evidence\"",
}
...
func (n *Ins) Insert(ctx context.Context, ex runtime.Executor) (Row, error) {
	...
	for i := 0; i < nInsertable; i++ {
		...
		switch i {
		case 0:
			args = append(args, n.row.GrantedAt)
		case 1:
			args = append(args, n.row.WithdrawnAt.Arg())
		case 2:
		...
		case 7:
			args = append(args, n.row.PolicyVersion)
		case 8:
			args = append(args, n.row.Evidence)
		}
	}
	var out Row
...
func Insert(ctx context.Context, ex runtime.Executor, r *Row) error {
	args := make([]any, 0, 9)
	args = append(args, r.GrantedAt)
	args = append(args, r.WithdrawnAt.Arg())
	args = append(args, r.ExpiresAt.Arg())
	args = append(args, r.ID)
	args = append(args, r.TenantID)
	args = append(args, r.IdentityID)
	args = append(args, r.Purpose)
	args = append(args, r.PolicyVersion)
	args = append(args, r.Evidence)
...
func (s *rowSource) Values() []any {
	r := &s.rows[s.i-1]
	s.buf[0] = &r.GrantedAt
	s.buf[1] = r.WithdrawnAt.Ptr()
	s.buf[2] = r.ExpiresAt.Ptr()
	s.buf[3] = &r.ID
	s.buf[4] = &r.TenantID
	s.buf[5] = &r.IdentityID
	s.buf[6] = &r.Purpose
	s.buf[7] = &r.PolicyVersion
	s.buf[8] = &r.Evidence
	return s.buf[:]
}
...
func InsertAll(ctx context.Context, ex runtime.Executor, rows []Row) (int64, error) {
	...
	return ex.CopyFrom(ctx, "consents", copyCols, src)
}
...
func (m *Mut) UpdateOp() (runtime.BatchOp, bool) {
	...
	for i := 0; i < nUpdatable; i++ {
		...
		switch i {
		case 0:
			args = append(args, m.row.GrantedAt)
		case 1:
			args = append(args, m.row.WithdrawnAt.Arg())
		case 2:
		...
		case 6:
			args = append(args, m.row.PolicyVersion)
		case 7:
			args = append(args, m.row.Evidence)
		}
	}
