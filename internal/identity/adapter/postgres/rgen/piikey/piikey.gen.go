...
type Row struct {
	CreatedAt  time.Time
	ShreddedAt runtime.Null[time.Time]
	ID         [16]byte
	TenantID   [16]byte
	KeyEnc     []byte
	KmsKeyRef  runtime.Null[string]
}
...
var (
	CreatedAt  = TimeCol{0}
	ShreddedAt = NullTimeCol{1}
	ID         = UUIDCol{2}
	TenantID   = UUIDCol{3}
	KeyEnc     = BytesCol{4}
	KmsKeyRef  = NullTextCol{5}
)
...
func (q Query) ShreddedAtEq(v time.Time) Query    { return q.Where(ShreddedAt.Eq(v)) }
func (q Query) ShreddedAtNotEq(v time.Time) Query { return q.Where(ShreddedAt.NotEq(v)) }
func (q Query) ShreddedAtGt(v time.Time) Query    { return q.Where(ShreddedAt.Gt(v)) }
func (q Query) ShreddedAtGte(v time.Time) Query   { return q.Where(ShreddedAt.Gte(v)) }
func (q Query) ShreddedAtLt(v time.Time) Query    { return q.Where(ShreddedAt.Lt(v)) }
func (q Query) ShreddedAtLte(v time.Time) Query   { return q.Where(ShreddedAt.Lte(v)) }
func (q Query) ShreddedAtIsNull() Query           { return q.Where(ShreddedAt.IsNull()) }
func (q Query) ShreddedAtIsNotNull() Query        { return q.Where(ShreddedAt.IsNotNull()) }
...
func (q Query) KeyEncEq(v []byte) Query           { return q.Where(KeyEnc.Eq(v)) }
func (q Query) KeyEncNotEq(v []byte) Query        { return q.Where(KeyEnc.NotEq(v)) }
func (q Query) KmsKeyRefEq(v string) Query        { return q.Where(KmsKeyRef.Eq(v)) }
func (q Query) KmsKeyRefNotEq(v string) Query     { return q.Where(KmsKeyRef.NotEq(v)) }
func (q Query) KmsKeyRefGt(v string) Query        { return q.Where(KmsKeyRef.Gt(v)) }
func (q Query) KmsKeyRefGte(v string) Query       { return q.Where(KmsKeyRef.Gte(v)) }
func (q Query) KmsKeyRefLt(v string) Query        { return q.Where(KmsKeyRef.Lt(v)) }
func (q Query) KmsKeyRefLte(v string) Query       { return q.Where(KmsKeyRef.Lte(v)) }
func (q Query) KmsKeyRefEqLower(v string) Query   { return q.Where(KmsKeyRef.EqLower(v)) }
func (q Query) KmsKeyRefLike(v string) Query      { return q.Where(KmsKeyRef.Like(v)) }
func (q Query) KmsKeyRefILike(v string) Query     { return q.Where(KmsKeyRef.ILike(v)) }
func (q Query) KmsKeyRefIn(v ...string) Query     { return q.Where(KmsKeyRef.In(v...)) }
func (q Query) KmsKeyRefNotIn(v ...string) Query  { return q.Where(KmsKeyRef.NotIn(v...)) }
func (q Query) KmsKeyRefIsNull() Query            { return q.Where(KmsKeyRef.IsNull()) }
func (q Query) KmsKeyRefIsNotNull() Query         { return q.Where(KmsKeyRef.IsNotNull()) }

const selectPrefix = `SELECT "created_at", "shredded_at", "id", "tenant_id", "key_enc", "kms_key_ref" FROM "pii_keys"`
const countPrefix = `SELECT count(*) FROM "pii_keys"`
const existsPrefix = `SELECT 1 FROM "pii_keys"`
...
var orderTable = [nCols][4]string{
	...
	{ // shredded_at
		"\"shredded_at\"",
		"\"shredded_at\" DESC",
		"\"shredded_at\" ASC NULLS FIRST",
		"\"shredded_at\" DESC NULLS LAST",
	},
	{ // id
		"\"id\"",
		"\"id\" DESC",
	...
	{ // key_enc
		"\"key_enc\"",
		"\"key_enc\" DESC",
		"\"key_enc\" ASC NULLS FIRST",
		"\"key_enc\" DESC NULLS LAST",
	},
	{ // kms_key_ref
		"\"kms_key_ref\"",
		"\"kms_key_ref\" DESC",
		"\"kms_key_ref\" ASC NULLS FIRST",
		"\"kms_key_ref\" DESC NULLS LAST",
	},
}
...
var fragTable = [6][28]runtime.Frag{
	...
	{ // shredded_at
		...
		{},
		{},
		{A: "\"shredded_at\" IS NULL", B: ""},
		{A: "\"shredded_at\" IS NOT NULL", B: ""},
		{},
		{},
	},
	{ // id
	...
	{ // kms_key_ref
		...
		{A: "\"kms_key_ref\" < $", B: ""},
		{A: "\"kms_key_ref\" <= $", B: ""},
		{A: "lower(\"kms_key_ref\") = lower($", B: ")"},
		{A: "\"kms_key_ref\" LIKE $", B: ""},
		{A: "\"kms_key_ref\" ILIKE $", B: ""},
		{},
		{},
		{},
		{},
		...
		{},
		{},
		{A: "\"kms_key_ref\" IS NULL", B: ""},
		{A: "\"kms_key_ref\" IS NOT NULL", B: ""},
		{},
		{},
	},
}
...
var defaultOrder = [1]runtime.Tok{
	runtime.MakeOrder(runtime.Asc, 2), // id
}
...
func scan(rv [][]byte, r *Row, sl *runtime.Slab) error {
	...
	r.ShreddedAt = runtime.Nullable(rv[1], runtime.Timestamptz)
	copy(r.ID[:], rv[2])
	copy(r.TenantID[:], rv[3])
	r.KeyEnc = runtime.Bytes(rv[4])
	r.KmsKeyRef = runtime.NullText(rv[5], sl)
	return nil
}
...
const insertSQL = `INSERT INTO "pii_keys" ("created_at", "shredded_at", "id", "tenant_id", "key_enc", "kms_key_ref") VALUES ($1, $2, $3, $4, $5, $6) RETURNING "created_at", "shredded_at", "id", "tenant_id", "key_enc", "kms_key_ref"`

const updatePrefix = `UPDATE "pii_keys" SET `
const deletePrefix = `DELETE FROM "pii_keys"`
...
const (
	...
	dShreddedAt uint64 = 1 << 1
	dTenantID   uint64 = 1 << 2
	dKeyEnc     uint64 = 1 << 3
	dKmsKeyRef  uint64 = 1 << 4
)
...
var setFrags = [nUpdatable]runtime.Frag{
	...
	{A: "\"shredded_at\" = $", B: ""}, // shredded_at
	{A: "\"tenant_id\" = $", B: ""},   // tenant_id
	{A: "\"key_enc\" = $", B: ""},     // key_enc
	{A: "\"kms_key_ref\" = $", B: ""}, // kms_key_ref
}
...
var exprFrags = [nUpdatable]runtime.Frag{
	...
	{A: "\"shredded_at\" = now()", B: ""}, // shredded_at = the database's clock
	{},                                    // tenant_id has no server-side form
	{},                                    // key_enc has no server-side form
	{},                                    // kms_key_ref has no server-side form
}
...
const (
	...
	iShreddedAt uint64 = 1 << 1
	iID         uint64 = 1 << 2
	iTenantID   uint64 = 1 << 3
	iKeyEnc     uint64 = 1 << 4
	iKmsKeyRef  uint64 = 1 << 5
)
...
const insPrefix = "INSERT INTO \"pii_keys\""
const insReturning = " RETURNING \"created_at\", \"shredded_at\", \"id\", \"tenant_id\", \"key_enc\", \"kms_key_ref\""
...
const updReturning = " RETURNING \"created_at\", \"shredded_at\", \"id\", \"tenant_id\", \"key_enc\", \"kms_key_ref\""
...
func (m *Mut) SetShreddedAt(v time.Time) {
	m.row.ShreddedAt = runtime.Null[time.Time]{V: v, Valid: true}
	m.dirty |= dShreddedAt
	m.expr &^= dShreddedAt
}

// SetShreddedAtNow assigns the database's clock, not this process's.
//
// Binding time.Now() instead would record when THIS server
// thought it was. Servers skew, so rows written seconds apart
// can land out of order, and an event stamped by a fast clock
// reads as preceding the thing that caused it.
func (m *Mut) SetShreddedAtNow() {
	m.expr |= dShreddedAt
	m.dirty &^= dShreddedAt
}

// SetShreddedAtNull writes SQL NULL. It is a separate method because a
// zero value and an absent value are different facts.
func (m *Mut) SetShreddedAtNull() {
	m.row.ShreddedAt = runtime.Null[time.Time]{}
	m.dirty |= dShreddedAt
	m.expr &^= dShreddedAt
}
...
func (m *Mut) SetKeyEnc(v []byte) {
	m.row.KeyEnc = v
	m.dirty |= dKeyEnc
	m.expr &^= dKeyEnc
}

func (m *Mut) SetKmsKeyRef(v string) {
	m.row.KmsKeyRef = runtime.Null[string]{V: v, Valid: true}
	m.dirty |= dKmsKeyRef
	m.expr &^= dKmsKeyRef
}

// SetKmsKeyRefNull writes SQL NULL. It is a separate method because a
// zero value and an absent value are different facts.
func (m *Mut) SetKmsKeyRefNull() {
	m.row.KmsKeyRef = runtime.Null[string]{}
	m.dirty |= dKmsKeyRef
	m.expr &^= dKmsKeyRef
}
...
func (n *Ins) SetShreddedAt(v time.Time) {
	n.row.ShreddedAt = runtime.Null[time.Time]{V: v, Valid: true}
	n.set |= iShreddedAt
}

// SetShreddedAtNull writes SQL NULL explicitly, which is not the same as
// leaving the column unset and taking its default.
func (n *Ins) SetShreddedAtNull() {
	n.row.ShreddedAt = runtime.Null[time.Time]{}
	n.set |= iShreddedAt
}
...
func (n *Ins) SetKeyEnc(v []byte) {
	n.row.KeyEnc = v
	n.set |= iKeyEnc
}

func (n *Ins) SetKmsKeyRef(v string) {
	n.row.KmsKeyRef = runtime.Null[string]{V: v, Valid: true}
	n.set |= iKmsKeyRef
}

// SetKmsKeyRefNull writes SQL NULL explicitly, which is not the same as
// leaving the column unset and taking its default.
func (n *Ins) SetKmsKeyRefNull() {
	n.row.KmsKeyRef = runtime.Null[string]{}
	n.set |= iKmsKeyRef
}
...
func assignable(i uint8, mask uint64) []string {
	...
	case 0:
		...
		if mask&(1<<1) != 0 {
			set = append(set, "shredded_at")
		}
		if mask&(1<<3) != 0 {
			set = append(set, "tenant_id")
		}
		if mask&(1<<4) != 0 {
			set = append(set, "key_enc")
		}
		if mask&(1<<5) != 0 {
			set = append(set, "kms_key_ref")
		}
	}
	return set
}
...
var assignFor = map[string]string{
	...
	"shredded_at": "\"shredded_at\" = EXCLUDED.\"shredded_at\"",
	"tenant_id":   "\"tenant_id\" = EXCLUDED.\"tenant_id\"",
	"key_enc":     "\"key_enc\" = EXCLUDED.\"key_enc\"",
	"kms_key_ref": "\"kms_key_ref\" = EXCLUDED.\"kms_key_ref\"",
}
...
func (n *Ins) Insert(ctx context.Context, ex runtime.Executor) (Row, error) {
	...
	for i := 0; i < nInsertable; i++ {
		...
		case 1:
			args = append(args, n.row.ShreddedAt.Arg())
		case 2:
			args = append(args, n.row.ID)
		case 3:
			args = append(args, n.row.TenantID)
		case 4:
			args = append(args, n.row.KeyEnc)
		case 5:
			args = append(args, n.row.KmsKeyRef.Arg())
		}
	}
	var out Row
...
func Insert(ctx context.Context, ex runtime.Executor, r *Row) error {
	...
	args = append(args, r.ShreddedAt.Arg())
	args = append(args, r.ID)
	args = append(args, r.TenantID)
	args = append(args, r.KeyEnc)
	args = append(args, r.KmsKeyRef.Arg())
...
func (s *rowSource) Values() []any {
	r := &s.rows[s.i-1]
	s.buf[0] = &r.CreatedAt
	s.buf[1] = r.ShreddedAt.Ptr()
	s.buf[2] = &r.ID
	s.buf[3] = &r.TenantID
	s.buf[4] = &r.KeyEnc
	s.buf[5] = r.KmsKeyRef.Ptr()
	return s.buf[:]
}
...
func InsertAll(ctx context.Context, ex runtime.Executor, rows []Row) (int64, error) {
	...
	return ex.CopyFrom(ctx, "pii_keys", copyCols, src)
}
...
func (m *Mut) UpdateOp() (runtime.BatchOp, bool) {
	...
	for i := 0; i < nUpdatable; i++ {
		...
		case 1:
			args = append(args, m.row.ShreddedAt.Arg())
		case 2:
			args = append(args, m.row.TenantID)
		case 3:
			args = append(args, m.row.KeyEnc)
		case 4:
			args = append(args, m.row.KmsKeyRef.Arg())
		}
	}
