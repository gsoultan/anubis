...
// source: table realm_categories

package realmcategory
...
const selectPrefix = `SELECT "created_at", "sort_order", "id", "tenant_id", "realm_id", "code", "display_name" FROM "realm_categories"`
const countPrefix = `SELECT count(*) FROM "realm_categories"`
const existsPrefix = `SELECT 1 FROM "realm_categories"`
...
const insertSQL = `INSERT INTO "realm_categories" ("created_at", "sort_order", "id", "tenant_id", "realm_id", "code", "display_name") VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING "created_at", "sort_order", "id", "tenant_id", "realm_id", "code", "display_name"`

const updatePrefix = `UPDATE "realm_categories" SET `
const deletePrefix = `DELETE FROM "realm_categories"`
...
const insPrefix = "INSERT INTO \"realm_categories\""
const insReturning = " RETURNING \"created_at\", \"sort_order\", \"id\", \"tenant_id\", \"realm_id\", \"code\", \"display_name\""
...
const updReturning = " RETURNING \"created_at\", \"sort_order\", \"id\", \"tenant_id\", \"realm_id\", \"code\", \"display_name\""
...
var conflictSpecs = []string{
	" ON CONFLICT (\"id\")",
	" ON CONFLICT (\"id\", \"realm_id\")",
	" ON CONFLICT (\"realm_id\", \"code\")",
}
...
// OnConflictIDRealmID upserts on the unique index over (id, realm_id).
//
// The row that already exists keeps every column this insert did
// not assign. Follow with DoNothing() to leave it untouched
// entirely.
func (n *Ins) OnConflictIDRealmID() *Ins {
	n.conflict = 4
	return n
}

// OnConflictRealmIDCode upserts on the unique index over (realm_id, code).
//
// The row that already exists keeps every column this insert did
// not assign. Follow with DoNothing() to leave it untouched
// entirely.
func (n *Ins) OnConflictRealmIDCode() *Ins {
	n.conflict = 6
	return n
}
...
func (n *Ins) Insert(ctx context.Context, ex runtime.Executor) (Row, error) {
	...
	for i := 0; i < nInsertable; i++ {
		...
		case 4:
			args = append(args, n.row.RealmID)
		case 5:
...
func InsertAll(ctx context.Context, ex runtime.Executor, rows []Row) (int64, error) {
	...
	return ex.CopyFrom(ctx, "realm_categories", copyCols, src)
}
...
func (m *Mut) UpdateOp() (runtime.BatchOp, bool) {
	...
	for i := 0; i < nUpdatable; i++ {
		...
		case 3:
			args = append(args, m.row.RealmID)
		case 4:
