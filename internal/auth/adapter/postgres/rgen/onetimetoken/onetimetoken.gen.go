...
const selectPrefix = `SELECT "created_at", "expires_at", "id", "tenant_id", "kind", "token_hash", "payload" FROM "one_time_tokens"`
const countPrefix = `SELECT count(*) FROM "one_time_tokens"`
const existsPrefix = `SELECT 1 FROM "one_time_tokens"`
...
func scan(rv [][]byte, r *Row, sl *runtime.Slab) error {
	...
	r.Kind = sl.Str(rv[4])
	r.TokenHash = runtime.Bytes(rv[5])
	r.Payload = runtime.JSON(runtime.JSONB(rv[6], sl))
	return nil
}
...
func (q Query) bindPreds(b *binder) []any {
	v := b.vals[:0]
	var ns, nr, ntm, njs, nby, nar, nas uint8
...
const insertSQL = `INSERT INTO "one_time_tokens" ("created_at", "expires_at", "id", "tenant_id", "kind", "token_hash", "payload") VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING "created_at", "expires_at", "id", "tenant_id", "kind", "token_hash", "payload"`

const updatePrefix = `UPDATE "one_time_tokens" SET `
const deletePrefix = `DELETE FROM "one_time_tokens"`
...
const (
	...
	dExpiresAt uint64 = 1 << 1
	dTenantID  uint64 = 1 << 2
	dKind      uint64 = 1 << 3
	dTokenHash uint64 = 1 << 4
	dPayload   uint64 = 1 << 5
)
...
var exprFrags = [nUpdatable]runtime.Frag{
	...
	{},                                   // payload has no server-side form
}
...
const insPrefix = "INSERT INTO \"one_time_tokens\""
const insReturning = " RETURNING \"created_at\", \"expires_at\", \"id\", \"tenant_id\", \"kind\", \"token_hash\", \"payload\""
...
const updReturning = " RETURNING \"created_at\", \"expires_at\", \"id\", \"tenant_id\", \"kind\", \"token_hash\", \"payload\""
...
func InsertAll(ctx context.Context, ex runtime.Executor, rows []Row) (int64, error) {
	...
	return ex.CopyFrom(ctx, "one_time_tokens", copyCols, src)
}
