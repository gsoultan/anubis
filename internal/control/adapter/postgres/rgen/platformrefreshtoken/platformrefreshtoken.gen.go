...
// source: table platform_refresh_tokens

package platformrefreshtoken
...
var (
	...
	FamilyID       = UUIDCol{2}
	TokenHash      = BytesCol{3}
	CreatedAt      = TimeCol{4}
	ExpiresAt      = TimeCol{5}
	UsedAt         = NullTimeCol{6}
	RevokedAt      = NullTimeCol{7}
)
...
const selectPrefix = `SELECT "id", "platform_user_id", "family_id", "token_hash", "created_at", "expires_at", "used_at", "revoked_at" FROM "platform_refresh_tokens"`
const countPrefix = `SELECT count(*) FROM "platform_refresh_tokens"`
const existsPrefix = `SELECT 1 FROM "platform_refresh_tokens"`
...
func scan(rv [][]byte, r *Row, sl *runtime.Slab) error {
	...
	copy(r.FamilyID[:], rv[2])
	r.TokenHash = runtime.Bytes(rv[3])
	r.CreatedAt = runtime.Timestamptz(rv[4])
	r.ExpiresAt = runtime.Timestamptz(rv[5])
	r.UsedAt = runtime.Nullable(rv[6], runtime.Timestamptz)
...
const insertSQL = `INSERT INTO "platform_refresh_tokens" ("id", "platform_user_id", "family_id", "token_hash", "created_at", "expires_at", "used_at", "revoked_at") VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING "id", "platform_user_id", "family_id", "token_hash", "created_at", "expires_at", "used_at", "revoked_at"`

const updatePrefix = `UPDATE "platform_refresh_tokens" SET `
const deletePrefix = `DELETE FROM "platform_refresh_tokens"`
...
const (
	...
	dFamilyID       uint64 = 1 << 1
	dTokenHash      uint64 = 1 << 2
	dCreatedAt      uint64 = 1 << 3
	dExpiresAt      uint64 = 1 << 4
	dUsedAt         uint64 = 1 << 5
	dRevokedAt      uint64 = 1 << 6
)
...
var exprFrags = [nUpdatable]runtime.Frag{
	...
	{A: "\"used_at\" = now()", B: ""},    // used_at = the database's clock
...
const (
	...
	iFamilyID       uint64 = 1 << 2
	iTokenHash      uint64 = 1 << 3
	iCreatedAt      uint64 = 1 << 4
	iExpiresAt      uint64 = 1 << 5
	iUsedAt         uint64 = 1 << 6
	iRevokedAt      uint64 = 1 << 7
)
...
const insPrefix = "INSERT INTO \"platform_refresh_tokens\""
const insReturning = " RETURNING \"id\", \"platform_user_id\", \"family_id\", \"token_hash\", \"created_at\", \"expires_at\", \"used_at\", \"revoked_at\""
...
const updReturning = " RETURNING \"id\", \"platform_user_id\", \"family_id\", \"token_hash\", \"created_at\", \"expires_at\", \"used_at\", \"revoked_at\""
...
func (m *Mut) SetUsedAt(v time.Time) {
	m.row.UsedAt = runtime.Null[time.Time]{V: v, Valid: true}
	m.dirty |= dUsedAt
	m.expr &^= dUsedAt
}

// SetUsedAtNow assigns the database's clock, not this process's.
//
// Binding time.Now() instead would record when THIS server
// thought it was. Servers skew, so rows written seconds apart
// can land out of order, and an event stamped by a fast clock
// reads as preceding the thing that caused it.
func (m *Mut) SetUsedAtNow() {
	m.expr |= dUsedAt
	m.dirty &^= dUsedAt
}

// SetUsedAtNull writes SQL NULL. It is a separate method because a
// zero value and an absent value are different facts.
func (m *Mut) SetUsedAtNull() {
	m.row.UsedAt = runtime.Null[time.Time]{}
	m.dirty |= dUsedAt
	m.expr &^= dUsedAt
}
...
func (n *Ins) SetUsedAt(v time.Time) {
	n.row.UsedAt = runtime.Null[time.Time]{V: v, Valid: true}
	n.set |= iUsedAt
}

// SetUsedAtNull writes SQL NULL explicitly, which is not the same as
// leaving the column unset and taking its default.
func (n *Ins) SetUsedAtNull() {
	n.row.UsedAt = runtime.Null[time.Time]{}
	n.set |= iUsedAt
}
...
// OnConflictTokenHash upserts on the unique index over (token_hash).
//
// The row that already exists keeps every column this insert did
// not assign. Follow with DoNothing() to leave it untouched
// entirely.
func (n *Ins) OnConflictTokenHash() *Ins {
	n.conflict = 4
	return n
}
...
var assignFor = map[string]string{
	...
	"used_at":          "\"used_at\" = EXCLUDED.\"used_at\"",
...
func (n *Ins) Insert(ctx context.Context, ex runtime.Executor) (Row, error) {
	...
	for i := 0; i < nInsertable; i++ {
		...
		case 6:
			args = append(args, n.row.UsedAt.Arg())
		case 7:
...
func (s *rowSource) Values() []any {
	...
	s.buf[4] = &r.CreatedAt
	s.buf[5] = &r.ExpiresAt
	s.buf[6] = r.UsedAt.Ptr()
	s.buf[7] = r.RevokedAt.Ptr()
	return s.buf[:]
}
...
func InsertAll(ctx context.Context, ex runtime.Executor, rows []Row) (int64, error) {
	...
	return ex.CopyFrom(ctx, "platform_refresh_tokens", copyCols, src)
}
...
func (m *Mut) UpdateOp() (runtime.BatchOp, bool) {
	...
	for i := 0; i < nUpdatable; i++ {
		...
		case 5:
			args = append(args, m.row.UsedAt.Arg())
		case 6:
...
const Table = "platform_refresh_tokens"
