...
// source: table scope_sync_runs

package scopesyncrun
...
func (q Query) FinishedAtEq(v time.Time) Query         { return q.Where(FinishedAt.Eq(v)) }
func (q Query) FinishedAtNotEq(v time.Time) Query      { return q.Where(FinishedAt.NotEq(v)) }
func (q Query) FinishedAtGt(v time.Time) Query         { return q.Where(FinishedAt.Gt(v)) }
func (q Query) FinishedAtGte(v time.Time) Query        { return q.Where(FinishedAt.Gte(v)) }
func (q Query) FinishedAtLt(v time.Time) Query         { return q.Where(FinishedAt.Lt(v)) }
func (q Query) FinishedAtLte(v time.Time) Query        { return q.Where(FinishedAt.Lte(v)) }
func (q Query) FinishedAtIsNull() Query                { return q.Where(FinishedAt.IsNull()) }
func (q Query) FinishedAtIsNotNull() Query             { return q.Where(FinishedAt.IsNotNull()) }
...
func (q Query) SourceIDEq(v [16]byte) Query            { return q.Where(SourceID.Eq(v)) }
func (q Query) SourceIDNotEq(v [16]byte) Query         { return q.Where(SourceID.NotEq(v)) }
func (q Query) SourceIDIn(v ...[16]byte) Query         { return q.Where(SourceID.In(v...)) }
func (q Query) SourceIDNotIn(v ...[16]byte) Query      { return q.Where(SourceID.NotIn(v...)) }
func (q Query) DryEq(v bool) Query                     { return q.Where(Dry.Eq(v)) }
func (q Query) DryNotEq(v bool) Query                  { return q.Where(Dry.NotEq(v)) }
...
func (q Query) ReportContains(v runtime.JSON) Query    { return q.Where(Report.Contains(v)) }
func (q Query) ReportContainedBy(v runtime.JSON) Query { return q.Where(Report.ContainedBy(v)) }
func (q Query) ReportHasAnyKey(v ...string) Query      { return q.Where(Report.HasAnyKey(v...)) }
func (q Query) ReportHasAllKeys(v ...string) Query     { return q.Where(Report.HasAllKeys(v...)) }

const selectPrefix = `SELECT "started_at", "finished_at", "id", "source_id", "dry", "status", "report" FROM "scope_sync_runs"`
const countPrefix = `SELECT count(*) FROM "scope_sync_runs"`
const existsPrefix = `SELECT 1 FROM "scope_sync_runs"`
...
var orderTable = [nCols][4]string{
	...
	{ // report
		"\"report\"",
		"\"report\" DESC",
		"\"report\" ASC NULLS FIRST",
		"\"report\" DESC NULLS LAST",
	},
}
...
var fragTable = [7][28]runtime.Frag{
	...
	{ // finished_at
		...
		{},
		{},
		{A: "\"finished_at\" IS NULL", B: ""},
		{A: "\"finished_at\" IS NOT NULL", B: ""},
		{},
		{},
	},
	{ // id
...
func scan(rv [][]byte, r *Row, sl *runtime.Slab) error {
	r.StartedAt = runtime.Timestamptz(rv[0])
	r.FinishedAt = runtime.Nullable(rv[1], runtime.Timestamptz)
	copy(r.ID[:], rv[2])
	copy(r.SourceID[:], rv[3])
	r.Dry = runtime.Bool(rv[4])
	r.Status = sl.Str(rv[5])
	r.Report = runtime.JSON(runtime.JSONB(rv[6], sl))
	return nil
}
...
const insertSQL = `INSERT INTO "scope_sync_runs" ("started_at", "finished_at", "id", "source_id", "dry", "status", "report") VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING "started_at", "finished_at", "id", "source_id", "dry", "status", "report"`

const updatePrefix = `UPDATE "scope_sync_runs" SET `
const deletePrefix = `DELETE FROM "scope_sync_runs"`

// Dirty bits. One per updatable column; the set of them is an UPDATE's
// identity, exactly as a token stream is a SELECT's.
const (
	dStartedAt  uint64 = 1 << 0
	dFinishedAt uint64 = 1 << 1
	dSourceID   uint64 = 1 << 2
	dDry        uint64 = 1 << 3
	dStatus     uint64 = 1 << 4
	dReport     uint64 = 1 << 5
)
...
var exprFrags = [nUpdatable]runtime.Frag{
	{A: "\"started_at\" = now()", B: ""},  // started_at = the database's clock
	{A: "\"finished_at\" = now()", B: ""}, // finished_at = the database's clock
	{},                                    // source_id has no server-side form
	{},                                    // dry has no server-side form
	{},                                    // status has no server-side form
	{},                                    // report has no server-side form
}
...
const (
	iStartedAt  uint64 = 1 << 0
	iFinishedAt uint64 = 1 << 1
	iID         uint64 = 1 << 2
	iSourceID   uint64 = 1 << 3
	iDry        uint64 = 1 << 4
	iStatus     uint64 = 1 << 5
	iReport     uint64 = 1 << 6
)
...
const insPrefix = "INSERT INTO \"scope_sync_runs\""
const insReturning = " RETURNING \"started_at\", \"finished_at\", \"id\", \"source_id\", \"dry\", \"status\", \"report\""
...
const updReturning = " RETURNING \"started_at\", \"finished_at\", \"id\", \"source_id\", \"dry\", \"status\", \"report\""
...
// SetStartedAtNow assigns the database's clock, not this process's.
//
...
func (m *Mut) SetStartedAtNow() {
	m.expr |= dStartedAt
	m.dirty &^= dStartedAt
}

func (m *Mut) SetFinishedAt(v time.Time) {
	m.row.FinishedAt = runtime.Null[time.Time]{V: v, Valid: true}
	m.dirty |= dFinishedAt
	m.expr &^= dFinishedAt
}

// SetFinishedAtNow assigns the database's clock, not this process's.
//
// Binding time.Now() instead would record when THIS server
// thought it was. Servers skew, so rows written seconds apart
// can land out of order, and an event stamped by a fast clock
// reads as preceding the thing that caused it.
func (m *Mut) SetFinishedAtNow() {
	m.expr |= dFinishedAt
	m.dirty &^= dFinishedAt
}

// SetFinishedAtNull writes SQL NULL. It is a separate method because a
// zero value and an absent value are different facts.
func (m *Mut) SetFinishedAtNull() {
	m.row.FinishedAt = runtime.Null[time.Time]{}
	m.dirty |= dFinishedAt
	m.expr &^= dFinishedAt
}

func (m *Mut) SetSourceID(v [16]byte) {
	m.row.SourceID = v
	m.dirty |= dSourceID
	m.expr &^= dSourceID
}

func (m *Mut) SetDry(v bool) {
	m.row.Dry = v
	m.dirty |= dDry
	m.expr &^= dDry
}
...
func (m *Mut) SetReport(v runtime.JSON) {
	m.row.Report = v
	m.dirty |= dReport
	m.expr &^= dReport
}
...
func (n *Ins) SetFinishedAt(v time.Time) {
	n.row.FinishedAt = runtime.Null[time.Time]{V: v, Valid: true}
	n.set |= iFinishedAt
}

// SetFinishedAtNull writes SQL NULL explicitly, which is not the same as
// leaving the column unset and taking its default.
func (n *Ins) SetFinishedAtNull() {
	n.row.FinishedAt = runtime.Null[time.Time]{}
	n.set |= iFinishedAt
}
...
func (n *Ins) SetSourceID(v [16]byte) {
	n.row.SourceID = v
	n.set |= iSourceID
}

func (n *Ins) SetDry(v bool) {
	n.row.Dry = v
	n.set |= iDry
}
...
func (n *Ins) SetReport(v runtime.JSON) {
	n.row.Report = v
	n.set |= iReport
}
...
var assignFor = map[string]string{
	...
	"finished_at": "\"finished_at\" = EXCLUDED.\"finished_at\"",
	"source_id":   "\"source_id\" = EXCLUDED.\"source_id\"",
	"dry":         "\"dry\" = EXCLUDED.\"dry\"",
	"status":      "\"status\" = EXCLUDED.\"status\"",
	"report":      "\"report\" = EXCLUDED.\"report\"",
}
...
func (n *Ins) Insert(ctx context.Context, ex runtime.Executor) (Row, error) {
	...
	for i := 0; i < nInsertable; i++ {
		...
		case 1:
			args = append(args, n.row.FinishedAt.Arg())
		case 2:
			args = append(args, n.row.ID)
		case 3:
			args = append(args, n.row.SourceID)
		case 4:
			args = append(args, n.row.Dry)
		case 5:
			args = append(args, n.row.Status)
		case 6:
			args = append(args, n.row.Report)
		}
	}
	var out Row
...
func Insert(ctx context.Context, ex runtime.Executor, r *Row) error {
	...
	args = append(args, r.FinishedAt.Arg())
	args = append(args, r.ID)
	args = append(args, r.SourceID)
	args = append(args, r.Dry)
	args = append(args, r.Status)
	args = append(args, r.Report)
...
func InsertAll(ctx context.Context, ex runtime.Executor, rows []Row) (int64, error) {
	...
	return ex.CopyFrom(ctx, "scope_sync_runs", copyCols, src)
}
...
func (m *Mut) UpdateOp() (runtime.BatchOp, bool) {
	...
	for i := 0; i < nUpdatable; i++ {
		...
		case 1:
			args = append(args, m.row.FinishedAt.Arg())
		case 2:
			args = append(args, m.row.SourceID)
		case 3:
			args = append(args, m.row.Dry)
		case 4:
			args = append(args, m.row.Status)
		case 5:
			args = append(args, m.row.Report)
		}
	}
