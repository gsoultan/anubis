...
func (q Query) CodeEqLower(v string) Query                 { return q.Where(Code.EqLower(v)) }
...
func (q Query) DisplayNameEqLower(v string) Query          { return q.Where(DisplayName.EqLower(v)) }
...
func (q Query) DefaultEffectEqLower(v string) Query        { return q.Where(DefaultEffect.EqLower(v)) }
...
var fragTable = [8][28]runtime.Frag{
	...
	{ // default_effect
		...
		{A: "\"default_effect\" < $", B: ""},
		{A: "\"default_effect\" <= $", B: ""},
		{A: "lower(\"default_effect\") = lower($", B: ")"},
...
var exprFrags = [nUpdatable]runtime.Frag{
	...
	{A: "\"sort_order\" = \"sort_order\" + 1", B: ""}, // sort_order = its own value plus one
	{}, // display_name has no server-side form
	{}, // default_effect has no server-side form
	{}, // status has no server-side form
	{}, // resolution has no server-side form
	{}, // ui_schema has no server-side form
}
...
// IncSortOrder adds one to the column's OWN value, in the database.
//
