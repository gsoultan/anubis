...
// source: table scope_node_types

package scopenodetype
...
func (q Query) ParentTypesContains(v ...string) Query { return q.Where(ParentTypes.Contains(v...)) }
func (q Query) ParentTypesContainedBy(v ...string) Query {
	return q.Where(ParentTypes.ContainedBy(v...))
}
...
var orderTable = [nCols][4]string{
	...
	{ // parent_types
		"\"parent_types\"",
		"\"parent_types\" DESC",
		"\"parent_types\" ASC NULLS FIRST",
		"\"parent_types\" DESC NULLS LAST",
	},
}
...
const (
	dAxisCode    uint64 = 1 << 0
	dDisplayName uint64 = 1 << 1
	dParentTypes uint64 = 1 << 2
)
...
var exprFrags = [nUpdatable]runtime.Frag{
	...
	{}, // parent_types has no server-side form
}
...
func (m *Mut) SetParentTypes(v []string) {
	m.row.ParentTypes = v
	m.dirty |= dParentTypes
	m.expr &^= dParentTypes
}
...
func (n *Ins) SetParentTypes(v []string) {
	n.row.ParentTypes = v
	n.set |= iParentTypes
}
...
var assignFor = map[string]string{
	...
	"parent_types": "\"parent_types\" = EXCLUDED.\"parent_types\"",
}
...
func Insert(ctx context.Context, ex runtime.Executor, r *Row) error {
	...
	args = append(args, r.ParentTypes)
...
func InsertAll(ctx context.Context, ex runtime.Executor, rows []Row) (int64, error) {
	...
	return ex.CopyFrom(ctx, "scope_node_types", copyCols, src)
}
