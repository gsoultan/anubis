...
func (q Query) ChangedAtEq(v time.Time) Query     { return q.Where(ChangedAt.Eq(v)) }
func (q Query) ChangedAtNotEq(v time.Time) Query  { return q.Where(ChangedAt.NotEq(v)) }
func (q Query) ChangedAtGt(v time.Time) Query     { return q.Where(ChangedAt.Gt(v)) }
func (q Query) ChangedAtGte(v time.Time) Query    { return q.Where(ChangedAt.Gte(v)) }
func (q Query) ChangedAtLt(v time.Time) Query     { return q.Where(ChangedAt.Lt(v)) }
func (q Query) ChangedAtLte(v time.Time) Query    { return q.Where(ChangedAt.Lte(v)) }
...
const selectPrefix = `SELECT "changed_at", "version", "tenant_id" FROM "catalog_version"`
const countPrefix = `SELECT count(*) FROM "catalog_version"`
const existsPrefix = `SELECT 1 FROM "catalog_version"`
...
var fragTable = [3][28]runtime.Frag{
	{ // changed_at
		{}, // opNone
		{A: "\"changed_at\" = $", B: ""},
		{A: "\"changed_at\" <> $", B: ""},
...
func scan(rv [][]byte, r *Row, sl *runtime.Slab) error {
	r.ChangedAt = runtime.Timestamptz(rv[0])
	r.Version = runtime.Int8(rv[1])
...
const insertSQL = `INSERT INTO "catalog_version" ("changed_at", "version", "tenant_id") VALUES ($1, $2, $3) RETURNING "changed_at", "version", "tenant_id"`

const updatePrefix = `UPDATE "catalog_version" SET `
const deletePrefix = `DELETE FROM "catalog_version"`

// Dirty bits. One per updatable column; the set of them is an UPDATE's
// identity, exactly as a token stream is a SELECT's.
const (
	dChangedAt uint64 = 1 << 0
	dVersion   uint64 = 1 << 1
)
...
var exprFrags = [nUpdatable]runtime.Frag{
	{A: "\"changed_at\" = now()", B: ""},        // changed_at = the database's clock
	{A: "\"version\" = \"version\" + 1", B: ""}, // version = its own value plus one
}
...
const insPrefix = "INSERT INTO \"catalog_version\""
const insReturning = " RETURNING \"changed_at\", \"version\", \"tenant_id\""
...
const updReturning = " RETURNING \"changed_at\", \"version\", \"tenant_id\""
...
func MutateKey(tenantID [16]byte) Mut {
	return Mut{row: Row{
		TenantID: tenantID,
	}}
}
...
func (m *Mut) SetChangedAt(v time.Time) {
	m.row.ChangedAt = v
	m.dirty |= dChangedAt
	m.expr &^= dChangedAt
}

// SetChangedAtNow assigns the database's clock, not this process's.
//
...
// IncVersion adds one to the column's OWN value, in the database.
//
...
func (n *Ins) SetChangedAt(v time.Time) {
	n.row.ChangedAt = v
	n.set |= iChangedAt
}
...
//	n.OnConflictTenantID().DoNothing()  // only on that one index
//
...
var assignFor = map[string]string{
	"changed_at": "\"changed_at\" = EXCLUDED.\"changed_at\"",
...
func (n *Ins) Insert(ctx context.Context, ex runtime.Executor) (Row, error) {
	...
	for i := 0; i < nInsertable; i++ {
		...
		switch i {
		case 0:
			args = append(args, n.row.ChangedAt)
		case 1:
...
func InsertAll(ctx context.Context, ex runtime.Executor, rows []Row) (int64, error) {
	...
	return ex.CopyFrom(ctx, "catalog_version", copyCols, src)
}
...
func (m *Mut) UpdateOp() (runtime.BatchOp, bool) {
	...
	for i := 0; i < nUpdatable; i++ {
		...
		switch i {
		case 0:
			args = append(args, m.row.ChangedAt)
		case 1:
...
func DeleteOp(tenantID [16]byte) runtime.BatchOp {
	return runtime.BatchOp{SQL: deleteSQL, Args: []any{tenantID}}
}
...
func Delete(ctx context.Context, ex runtime.Executor, tenantID [16]byte) error {
	n, err := ex.Exec(ctx, deleteSQL, []any{tenantID})
	if err != nil {
		return err
	}
	if n == 0 {
