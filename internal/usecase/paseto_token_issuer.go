...
func (t *pasetoTokenIssuer) Issue(ctx context.Context, in IssueInput) (*TokenPair, error) {
	...
	newID, err := t.refresh.CreateRefresh(ctx, repository.RefreshInput{
		SessionID:  s.ID,
		TenantID:   s.TenantID,
		FamilyID:   familyID,
...
func (t *pasetoTokenIssuer) ttls(ctx context.Context, s *repository.SessionView, clientID string) (access, refresh time.Duration, aud []string, appID string, err error) {
	...
	if clientID != "" {
		...
		if aerr != nil {
			return 0, 0, nil, "", domain.ErrInvalidArgument.With("client_id", "unknown application")
		}
		aud = []string{app.Slug}
		appID = app.ID
