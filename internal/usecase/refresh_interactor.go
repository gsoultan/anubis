...
func (u *refreshInteractor) Execute(ctx context.Context, in RefreshInput) (*TokenPair, error) {
	...
	var pair *TokenPair
	var deadClaim *repository.RefreshClaim
...
func (u *refreshInteractor) respondToFailedClaim(ctx context.Context, hash []byte) error {
	...
	case "consumed", "revoked":
		...
		u.audit.Emit(ctx, repository.AuditEvent{
			...
			Detail:    mustJSON(map[string]string{"family_id": info.FamilyID}),
		})
