...
type authzAdminInteractor struct {
	...
	perms   repository.PermissionCatalogRepository
...
func NewAuthzAdminInteractor(
	...
	return &authzAdminInteractor{
		guard: newAdminGuard(authz), roles: roles, perms: perms,
...
func (u *authzAdminInteractor) ListRoles(ctx context.Context, query string) ([]repository.RoleRecord, map[string][]string, map[string][]string, error) {
...
func (u *authzAdminInteractor) CreateRole(ctx context.Context, r repository.RoleRecord, parents, patterns []string) (*repository.RoleRecord, error) {
	p, err := u.guard.require(ctx, "anubis:role:admin")
	if err != nil {
		return nil, err
	}
	...
	if r.ApplicationSlug != "" {
		...
		if aerr != nil {
			return nil, domain.ErrInvalidArgument.With("application", r.ApplicationSlug)
		}
		appID = app.ID
	}
	var id string
...
func (u *authzAdminInteractor) UpdateRole(ctx context.Context, r repository.RoleRecord, parents, patterns []string) (*repository.RoleRecord, error) {
	p, err := u.guard.require(ctx, "anubis:role:admin")
	if err != nil {
		return nil, err
	}
...
func (u *authzAdminInteractor) GetRoleEffective(ctx context.Context, roleID string) ([]repository.EffectivePermissionRecord, error) {
...
func (u *authzAdminInteractor) ListPermissions(ctx context.Context, applicationSlug string, includeDeprecated bool) ([]repository.PermissionRecord, error) {
...
func (u *authzAdminInteractor) ListGrants(ctx context.Context, identityID string, includeRevoked bool) ([]repository.GrantRecord, []repository.GrantScopeRecord, error) {
	p, err := u.guard.require(ctx, "anubis:identity:read")
	if err != nil {
		return nil, nil, err
	}
...
func (u *authzAdminInteractor) CreateGrant(ctx context.Context, in repository.GrantCreate) (string, error) {
...
func (u *authzAdminInteractor) ListMemberships(ctx context.Context) ([]repository.MembershipRecord, []repository.MembershipEntryRecord, []repository.MembershipEntryScopeRecord, error) {
...
func (u *authzAdminInteractor) CreateMembership(ctx context.Context, name, description string) (*repository.MembershipRecord, error) {
	p, err := u.guard.require(ctx, "anubis:membership:admin")
	if err != nil {
		return nil, err
	}
	...
	return &repository.MembershipRecord{ID: id, Name: name, Description: description}, nil
}

func (u *authzAdminInteractor) SetMembershipEntries(ctx context.Context, membershipID string, entries []repository.MembershipEntryInput) (int, error) {
	p, err := u.guard.require(ctx, "anubis:membership:admin")
	if err != nil {
		return 0, err
	}
...
func (u *authzAdminInteractor) ApplyManifest(ctx context.Context, applicationSlug, manifestJSON string, dry bool) (string, int, error) {
	...
	if err != nil {
		return "", 0, domain.ErrNotFound.With("application", applicationSlug)
	}
	var m Manifest
	if err := json.Unmarshal([]byte(manifestJSON), &m); err != nil {
		return "", 0, domain.ErrInvalidArgument.With("manifest", "invalid JSON").Wrap(err)
	}
	...
	apply := func(ctx context.Context) error {
		...
		for _, mr := range m.Roles {
			...
			var roleID string
			if rerr != nil {
				roleID, rerr = u.roles.CreateRole(ctx, p.TenantID, repository.RoleRecord{
		...
		policies := make([]repository.RoutePolicyInput, 0, len(m.Routes))
		for _, rt := range m.Routes {
			...
			policies = append(policies, repository.RoutePolicyInput{
...
func validateManifest(m *Manifest) error {
	...
	for _, p := range m.Permissions {
		if p.Resource == "" || p.Action == "" {
			return domain.ErrInvalidArgument.With("manifest", "permission missing resource/action")
		}
	...
	for _, r := range m.Routes {
		...
		default:
			return domain.ErrInvalidArgument.With("route", r.PathPattern).With("effect", r.Effect)
		}
		if prev, dup := prio[r.Priority]; dup {
			return domain.ErrInvalidArgument.With("route", r.PathPattern).
